...
#define HXD_NO_HIGHLIGHT -1

#define HXD_COLOR_BLACK   "\x1b[30m"
...
#define HXD_BG_COLOR_GREEN "\x1b[46m"
#define HXD_BG_COLOR_RESET "\x1b[0m"
...
static void render_line(const int bytes_read, uint8_t *input_buffer, const int 
	offset, struct hxd_session *session, const int highlight_start, 
	const int highlight_end);
static inline void render_byte(const uint8_t b, const bool highlight);
static inline void render_ascii(const uint8_t b, const bool highlight);
